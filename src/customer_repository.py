...
class CustomerRepository:
    ...
    def mark_concurrent_users(self, user_ids: List[str]) -> int:
        """Mark users as 'concurrent' status in the customers table.

        Concurrent users are inactive (not renewed) but still connected
        to the network. This is a third status category beyond active/inactive.

        Args:
            user_ids: List of user_id strings to mark as concurrent.
        ...
        try:
            ...
            for user_id in user_ids:
                cursor.execute(
                    "UPDATE customers SET status = 'concurrent' WHERE user_id = %s",
                    (user_id,)
                )
            ...
            logger.info("Marked %d users as concurrent (of %d IDs provided)",
                        updated, len(user_ids))
            return updated

        except Exception as e:
            self._connection.rollback()
            raise CustomerRepositoryError(f"Failed to mark concurrent users: {e}") from e

    def reset_concurrent_status(self) -> int:
        """Reset all 'concurrent' statuses back to their numeric value before re-tagging.

        This ensures users who are no longer concurrent get their status corrected.
        ...
        try:
            cursor = self._connection.cursor()
            cursor.execute(
                "UPDATE customers SET status = '0' WHERE status = 'concurrent'"
            )
            ...
            if reset_count > 0:
                logger.info("Reset %d previously-concurrent users to inactive", reset_count)
            return reset_count

        except Exception as e:
            self._connection.rollback()
            raise CustomerRepositoryError(f"Failed to reset concurrent status: {e}") from e
